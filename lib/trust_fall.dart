...
class TrustFall {
  ...
  static Future<bool?> get canMockLocation async {
    final bool? canMockLocation =
        await _channel.invokeMethod('canMockLocation');
  ...
  static Future<bool> get isTrustFall async {
    ...
    if (isJailBroken == null || canMockLocation == null || isRealDevice == null)
      throw 'Could not determine whether your device is compromised';

    if (Platform.isAndroid) {
      final bool? isOnExternalStorage =
          await _channel.invokeMethod('isOnExternalStorage');

      if (isOnExternalStorage == null)
        throw 'Could not determine whether your application is on external storage';
